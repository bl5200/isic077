...
main(int argc, char **argv)
{
	...
	while((c = getopt(argc, argv, "a:b:hd:I:s:r:m:k:Dp:V:F:vx:y:z:")) != EOF) {
	  switch (c) {
	   case 'h':
		usage(argv[0]);
		exit(0);
...
{
   fprintf(stderr,
	"usage: %s [-v] [-D] -s <source ip> -a <source begain> -y <source end> \n"
	"          -d <destination ip> -b <destination begain> -z <source end> [-r <random seed>]\n"
